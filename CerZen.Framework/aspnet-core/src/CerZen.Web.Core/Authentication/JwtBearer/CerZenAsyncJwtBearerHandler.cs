...
{
    public class CerZenAsyncJwtBearerHandler : AuthenticationHandler<AsyncJwtBearerOptions>
    {
        ...
        /// </summary>
        /// <inheritdoc />
        public CerZenAsyncJwtBearerHandler(IOptionsMonitor<AsyncJwtBearerOptions> options,
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            try
            {
                ...
                List<Exception>? LogTokenValidationException(Exception ex)
                {
                ...
                foreach (var validator in Options.AsyncSecurityTokenValidators)
                {
                    if (validator.CanReadToken(token))
                    {
                        ...
                        SecurityToken? asyncValidatedToken = null;
                        try
                        {
                            (principal, asyncValidatedToken) =
                                await validator.ValidateToken(token, validationParameters);
                        }
                        catch (Exception ex)
                        {
                ...
                foreach (var validator in Options.SecurityTokenValidators)
                {
                    ...
                    if (validator.CanReadToken(token))
                    {
                        ...
                        return await OnTokenValidationSucceed(principal, validatedToken, token);
                    }
                }
        ...
        private async Task<AuthenticateResult> OnTokenValidationSucceed(ClaimsPrincipal principal,
            SecurityToken? validatedToken, string? token)
        {
        ...
        protected override async Task HandleChallengeAsync(AuthenticationProperties properties)
        {
            ...
            {
                Response.Headers.Add(HeaderNames.WWWAuthenticate, Options.Challenge);
            }
            else
            {
                ...
                Response.Headers.Add(HeaderNames.WWWAuthenticate, builder.ToString());
            }
        }
