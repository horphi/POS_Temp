...
namespace CerZen.ApiClient
{
    public class AbpApiClient : ISingletonDependency, IDisposable
    {
        ...
        public async Task<T> GetAnonymousAsync<T>(string endpoint, object queryParameters)
        {
            return await GetAsync<T>(endpoint, queryParameters, null, true);
        }
        ...
        private void AddHeaders(string accessToken)
        {
            ...
            _client.WithHeader("User-Agent", CerZenConsts.AbpApiClientUserAgent);
