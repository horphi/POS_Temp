...
using CerZen.Authentication.TwoFactor.Google;
...
using CerZen.Authorization.Impersonation;
...
using CerZen.Net.Sms;
using CerZen.Notifications;
using CerZen.Web.Models.Account;
using CerZen.Security;
using CerZen.Security.Recaptcha;
using CerZen.Sessions;
using CerZen.Url;
using CerZen.Web.Authentication.External;
using CerZen.Web.Security.Recaptcha;
using CerZen.Web.Session;
using CerZen.Web.Views.Shared.Components.TenantChange;
...
{
    public class AccountController : CerZenControllerBase
    {
            ...
            string returnUrlHash = "", string ss = "")
        {
            ...
            if (loginResult.User.ShouldChangePasswordOnNextLogin)
            {
                ...
                var expireDate =
                    Uri.EscapeDataString(Clock.Now.AddHours(1).ToString(CerZenConsts.DateTimeOffsetFormat));
        ...
        {
            return RedirectToAction("Index", "Home", new {area = "WebApp"});
        }

        public string GetAppHomeUrl()
        {
            return Url.Action("Index", "Home", new {area = "WebApp"});
        }
