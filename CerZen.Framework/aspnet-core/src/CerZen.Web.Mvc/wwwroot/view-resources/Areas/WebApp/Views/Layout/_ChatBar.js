...
var chat = {
    ...
    getFormattedMessages: function (messages) {
        $.each(messages, function (index, message) {
            ...
            if (message.message.startsWith('[image]')) {
                ...
                var imageUrl = abp.appPath + 'WebApp/Chat/GetImage?id=' + message.id + '&contentType=' + image.contentType;
                ...
                var fileUrl = abp.appPath + 'WebApp/Chat/GetFile?id=' + message.id + '&contentType=' + file.contentType;
                var uploadedFileMsg =
                    '<a href="' +
                    fileUrl +
    ...
    scrollToBottom: function () {
        const messageCount = chat.selectedUser.messages.length;
        chat.scrollTopChatPanel(messageCount * 100);
    },

    scrollTopChatPanel: function (amount) {
        $("#kt_drawer_chat_messenger_body").scrollTop(amount);
    },
        ...
        $('#kt_drawer_chat_messenger_body').on('scroll', function (e) {
            if (e.target.scrollTop === 0 && !chat.user.allPreviousMessagesLoaded) {
                chat.user.loadMessages(chat.selectedUser, () => {
                    ...
                    chat.scrollTopChatPanel(maxMessageCount * messageHeight);
                });
            }
        });
...
(function ($) {
    $(function () {
        ...
        var url = abp.appPath + 'WebApp/Chat/UploadFile';
