(function ($) {
  $(function () {
    ...
    _$auditLogFilterForm
      .find('input.date-range-picker')
      .daterangepicker(
    ...
    _auditLogService.getEntityHistoryObjectTypes().done(function (result) {
      $.each(result, function (index, item) {
        // Iterates through a collection
        $('#EntityTypeFullName').append(
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (exception) {
            ...
            if (exception) {
              $div.append(
                $('<i/>').addClass('fa fa-exclamation-triangle text-danger').attr('title', app.localize('HasError'))
              );
            } else {
              $div.append($('<i/>').addClass('fa fa-check-circle text-success').attr('title', app.localize('Success')));
            }
        ...
        },
        {
          ...
          render: function (browserInfo) {
            return $('<span/>').text(abp.utils.truncateStringWithPostfix(browserInfo, 32)).attr('title', browserInfo)[0]
              .outerHTML;
          },
        },
        {
    ...
    function createAuditLogRequestParams() {
      ...
      _$auditLogFilterForm.serializeArray().map(function (x) {
        prms[abp.utils.toCamelCase(x.name)] = x.value;
      });
    ...
    function createEntityChangeRequestParams() {
      var prms = {};
      _$entityChangeFilterForm.serializeArray().map(function (x) {
        prms[abp.utils.toCamelCase(x.name)] = x.value;
      });
    ...
    function showEntityChangeDetails(entityChange) {
      var entityChangeDetailModal = new app.ModalManager({
        viewUrl: abp.appPath + 'WebApp/AuditLogs/EntityChangeDetailModal',
    ...
    function showAuditLogDetails(auditLog) {
      ...
      $('#AuditLogDetailModal_ExecutionTime').html(
        moment(auditLog.executionTime).fromNow() +
          ' (' +
          moment(auditLog.executionTime).format('YYYY-MM-DD hh:mm:ss') +
          ')'
      );
    ...
    $('#ExportAuditLogsToExcelButton').click(function (e) {
      e.preventDefault();
      _auditLogService.getAuditLogsToExcel(createAuditLogRequestParams()).done(function (result) {
    ...
    $('#ExportEntityChangesToExcelButton').click(function (e) {
      e.preventDefault();
      _auditLogService.getEntityChangesToExcel(createEntityChangeRequestParams()).done(function (result) {
        app.downloadTempFile(result);
      });
    });
